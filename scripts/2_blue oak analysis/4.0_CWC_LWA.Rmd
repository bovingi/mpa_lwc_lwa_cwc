...
#LWA data: 
data_og_lwa1  <- read_csv(here::here("processed-data", paste0("analysis",datver,".csv")), show_col_types = FALSE) %>% 
...
data_lwa_cwc <- merge(data_lwa_cwc, data_og_lwa1, 
...
run_bootstrap <- function(data, time_season) {
  ...
  ci_values <- bootstrapped_results %>%
    summarize(
      slope = mean(mean_slope, na.rm = TRUE),
...
cwc_time_boot_df %>%
  ...
  geom_point(aes(y = slope, color = "Mean")) +
...
```{r}
wc_wk_centered_df <- data_lwa_cwc %>% 
  filter(time == "md")
#week 10: 
...
space_mod <- lm(lwa_g_cm2 ~ cwc, data =  wk)
summary(space_mod)
#not sig
...
highlighted_weeks <- c("03-08", "03-25")  # Example weeks

space_slope_se <- final_df_space_se %>%  
  mutate(week = case_when(
        week %in% c(10) ~ "2022-03-08",
...
library(grid)
#labels of slopes: 
y.grob1 <- textGrob("Slope (\U0394 LWarea/ \U0394 CWC)",
...
combined_slopes_wide <- cowplot::plot_grid(time_summary, NULL, space_slope_se, 
                                           # NULL, pv_slope,
                                   nrow = 1, 
...
ggsave(here("figures", "qudo figures", "CWC", "combined_plots_lwa_qudo"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
