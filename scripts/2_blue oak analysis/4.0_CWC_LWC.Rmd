...
```{r}
#CWC data: 
data_lwc_cwc <- read_csv(here("processed-data", paste0("cwc_analysis",datver,".csv")), show_col_types = F) %>%  
  filter(species %in% c("blue oak")) %>% 
  dplyr::select(tree, week,cwc, time_season, time) %>% 
  #mutate(lwc_mean = -1*lwc_mean) %>% 
  distinct() %>% 
...
#LWA data: 
data_og_lwc  <- read_csv(here::here("processed-data", paste0("analysis",datver,".csv")), show_col_types = FALSE) %>% 
  distinct() %>% 
  ...
  select(tree, week, lwc_mean, time_season, time, species, date_wp) %>% 
  distinct() %>% 
 # filter(week == 15) %>% 
  arrange(tree, week, time) 

data_lwc_cwc <- merge(data_lwc_cwc, data_og_lwc, 
                      by = c("tree", "week", "time_season", "time"),
                      all = TRUE) %>% 
  rename(lwc_mean = lwc_mean) %>% 
  group_by(tree, week) %>% 
  fill(c(cwc), .direction = "downup")
  
#take a look at the counts for each week: 
data_lwc_cwc %>% 
  drop_na(cwc, lwc_mean) %>% 
  group_by(week, time) %>% 
  summarise(n = n())
```
...
n_iterations <- 1000
n_sample_trees <- 43  # Adjust as needed based on the number of unique trees

# Define the function to calculate the slope for a specific tree
calculate_slope_for_tree <- function(tree_data) {
  model <- lm(lwc_mean ~ cwc, data = tree_data)
...
#model without site: 
space_mod_4 <- lmer(lwc_mean ~ cwc + (cwc|week), data =  data , REML = T)
...
for (x in unique_weeks) {
  ...
  for (i in 1:n_iterations) {
    ...
    model <- lm(lwc_mean ~ cwc, data = boot_sample)
...
final_df_all_cwc %>% write_csv(here::here("processed-data", "bootstrapped_df_cwc_lwc.csv"))
```

```{r}
final_df_all_cwc_lwc <- read_csv(here::here("processed-data", "bootstrapped_df_cwc_lwc.csv"))
```
#--------
...
```{r}
wc_wk_centered_df <- data_lwc_cwc %>% 
  filter(time == "md")
#week 10: 
...
space_mod <- lm(lwc_mean ~ cwc, data =  wk)
summary(space_mod)
#not sig
...
```{r, eval = F}
space_slope_bs <-  final_df_all_cwc_lwc %>%  
  mutate(week = case_when(
        week %in% c(10) ~ "2022-03-08",
        week %in% c(10) ~  "2022-03-15", 
        week %in% c(12) ~  "2022-03-25",
...
```{r}
time_summary <- final_df_all_cwc_lwc %>% 
   filter(!week %in% c(9)) %>% 
...
ggplot(aes(color = time_season)) + 
  geom_point(aes(y = mean, x = time_season), size =3) +
...
```{r}
legend_plot <- data_lwc_cwc  %>% 
  ungroup() %>% 
  add_row(date_wp = as.Date("2022-05-03"), time = "md", cwc = .05, lwc_mean = -.3) %>% 
  filter(date_wp %in% c(
         "2022-03-08",
        # "2022-03-15", 
         "2022-03-25",
...
library(grid)
#labels of slopes: 
y.grob1 <- textGrob("Slope (\U0394 LWmass/ \U0394 CWC)",
...
ggsave(here("figures", "qudo figures", "CWC", "combined_plots_lwc_qudo_bs"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
...
<!-- dcwc_df <- merge(dcwc_space, data_og_lwc) -->
...
<!-- sig_mods_whole_lwc <- model_results %>%  -->
<!--   filter(p.value < 0.05)  -->
<!-- sig_mods_whole_lwc -->
...
<!-- # Number of random trees to sample -->
<!-- n_sample_trees <- 34  # Adjust as needed based on the number of unique trees -->
...
<!-- sig_mods_before_lwc <- model_results %>%  -->
<!--   filter(p.value < 0.05)  -->
<!-- sig_mods_before_lwc -->

<!-- dcwc_df_no_outliers %>%  -->
<!--   filter((date_wp %in% c(sig_mods_before_lwc$date_wp) & time %in% c(sig_mods_before_lwc$time))) %>%  -->
<!--                    ggplot(aes(y = slope,  -->
<!--                               x = lwc_mean,  -->
<!--                               color = time)) + -->
<!--                    geom_smooth(method = "lm") + -->
...
<!--   bootstrapped_slopes_list %>%  -->
<!--   keep(~ is.data.frame(.)) -->
<!-- ) -->
<!-- #bind_rows(bootstrapped_slopes_list) -->
...
<!-- sig_mods_may_lwc <- model_results %>%  -->
<!--   filter(p.value < 0.05)  -->
<!--   #filter(p.value < 0.1) -->
<!-- sig_mods_may_lwc -->

<!-- dcwc_df_no_outliers %>% -->
<!--   filter((date_wp %in% c(sig_mods_may_lwc$date_wp) & time %in% c(sig_mods_may_lwc$time))) %>% -->
<!--                    ggplot(aes(y = slope, -->
<!--                               x = lwc_mean, -->
<!--                               color = time)) + -->
<!--                    geom_smooth(method = "lm") + -->
...
<!-- ```{r} -->
<!-- sig_mods_all_lwc <- bind_rows(sig_mods_all_lwc %>% mutate(var = "lwc", -->
<!--                                                       timeframe = "whole timeframe"),  -->
<!--                           sig_mods_whole_lwc %>% mutate(var = "lwc", -->
<!--                                                       timeframe = "after leaf expansion"),  -->
<!--                           sig_mods_before_lwc%>% mutate(var = "lwc", -->
<!--                                                       timeframe = "before leaf expansion"),  -->
<!--                           sig_mods_may_lwc%>% mutate(var = "lwc", -->
<!--                                                       timeframe = "just May") -->
<!--                           ) %>%  -->
...
<!-- write_csv(sig_mods_all_lwc, here("processed-data", "model results", "dcwc_mods_lwc.csv")) -->
<!-- ``` -->