...
data_qudo_lma <- read_csv(here("data", paste0("wp_wc_rwc_",datver,".csv"))) %>% 
 # filter(time %in% c("md")) %>% 
  ...
  mutate(lwa_g_cm2 = lwa_g_cm2_new, #use the 'new' version, increases sample size because this includes estimates from bulk data.
         ...
         lwa_g_cm2 = -1*lwa_g_cm2) %>% 
  filter(species %in% c("blue oak")) %>% 
  select(week, time, tree, species, lma_g_cm2, lwa_g_cm2, lwc_mean) %>% 
  distinct()

data_qudo_lma <- read_csv(here("processed-data", paste0("analysis_bothspp",datver,".csv")))  %>% 
  filter(species %in% c("blue oak")) %>% 
  select(week, time, tree, species, lma_g_cm2, lwa_g_cm2, lwc_mean) %>% 
  distinct() 

df0 <- merge(data_qudo_lma, data_og_cwc,
...
calculate_slope_for_tree <- function(tree_data) {
  model <- lm(lwc_mean ~ cw_area, data = tree_data)
...
data <- common_trees_data %>% 
  filter(time_season == "after leafout") %>% 
  select(tree, week, lwc_mean, cw_area) %>% 
  distinct() %>% 
  drop_na(lwc_mean, cw_area) %>% 
  group_by(tree) %>% 
  mutate(COUNT = n()) %>% 
...
data <- df %>% 
  filter(time == "md") %>% 
  select(tree, week, lwc_mean, cw_area, site) %>% 
  distinct() %>% 
  drop_na(cw_area, lwc_mean)
...
#model without site: 
space_mod_4 <- lmer(lwc_mean ~ cw_area + (cw_area|week), data =  data , REML = T)
...
for (x in unique_weeks) {
  ...
  for (i in 1:n_iterations) {
    ...
    # Calculate the slope for the bootstrapped sample
    model <- lm(lwc_mean ~ cw_area, data = boot_sample)
...
final_df_all_cw_area %>% write_csv(here::here("processed-data", "bootstrapped_df_qudo_lwc_cw_area.csv"))
```
#------------

#START HERE to skip bootstrapping: 
```{r}
final_df_all_cw_area <- read_csv(here::here("processed-data", "bootstrapped_df_qudo_lwc_cw_area.csv"))
```
...
wk_all_wide <- wk_all %>% 
  ...
  pivot_wider(names_from=time, 
              values_from = c(cw_area, lwc_mean), 
              values_fn = function(x) mean(x, na.rm = TRUE)
              ) %>% 
    dplyr::group_by(tree, week) %>%
  fill(c(cw_area_md, cw_area_pd, lwc_mean_pd, lwc_mean_md), .direction = "downup") %>%
  dplyr::ungroup()%>% 
 # select(-date_wp) %>% 
  distinct()

wk_all_long_cw_area <- wk_all_wide %>% 
  drop_na(cw_area_pd, cw_area_md, lwc_mean_md, lwc_mean_pd) %>% 
...
ggsave(here("figures", "qudo figures", "cw_area", "cw_area", "nice_space_lwc_cw_area"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
space_mod <- lmer(lwc_mean ~ cw_area + (cw_area|week), data = space_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(lwc_mean ~ cw_area  + (cw_area|week) + (1|site), data = space_df_nosite,  REML = T)
...
```{r, echo=FALSE}
space_mod_4 <- lmer(lwc_mean ~ cw_area + (cw_area|week) + (1|site), data =  wc_wk_centered_df, REML = T)
...
```{r}
mod <- lmer(lwc_mean ~ cw_area*week + (1|site), data =wc_wk_centered_df %>% 
              mutate(week = as.factor(week)))
...
space_mod <- lmer(lwc_mean ~ cw_area + (1|site), data =  wk,  REML = T)
summary(space_mod)
#not sig
...
space_mod <- lm(lwc_mean ~ cw_area, data =  wk)
summary(space_mod)
#not sig
...
space_mod <- lmer(lwc_mean ~ cw_area + (1|week) + (cw_area|tree), data = space_df_nosite, REML = F)
#summary(space_mod)
AIC(space_mod)

space_mod_site <- lmer(lwc_mean ~ cw_area  + (1|week) + (cw_area|tree) + (1|site), data = space_df_nosite,  REML = F)
...
time_season_mod <- lmer(lwc_mean ~ cw_area + time_season + (1|week) + (cw_area|tree), data = wc_qudo_md_df)

time_season_mod_df <- broom::tidy(time_season_mod)

#with separate periods: 
time_season_mod_int <- lmer(lwc_mean ~ cw_area*time_season + (1|week) + (cw_area|tree), data = wc_qudo_md_df)

time_season_mod_int_df <- broom::tidy(time_season_mod)

time_mod <- lmer(lwc_mean ~ cw_area + (1|week) + (cw_area|tree), data = wc_qudo_md_df)
#time_mod <- lmer(lwc_mean ~ cw_area + (cw_area|tree), data = wc_qudo_md_df)
...
lin_time_mod <- lmer(lwc_mean ~ cw_area + (1|week) + (cw_area|tree), data = lin_wc_qudo_md_df)
...
bef_time_mod <- lmer(lwc_mean ~ cw_area + (1|week) + (cw_area|tree), data = bef_wc_qudo_md_df, REML = T)
...
write_csv(cw_area_mods, here::here("processed-data", "model results", "cw_area_lwc_mods.csv"))
```
...
legend_plot <- df  %>% 
  ungroup() %>% 
  add_row(date_wp = as.Date("2022-05-03"), time = "md", cw_area = .05, lwc_mean = -.3) %>% 
...
ggsave(here("figures", "qudo figures", "cw_area", "combined_plots_all_qudo_lwc_bs"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
