...
wc_all_raw <- read_csv(here("processed-data", paste0("analysis",datver,".csv")), show_col_types = F) %>% 
  filter(!(week %in% 33)) #week 33 is screwy for LWA for some reason, remove for now 
```
...
```{r}
plotly::ggplotly(lin_nice <- qudo_wc_linear %>% 
  ggplot(aes(y = water_potential, 
             x = lwa_g_cm2_new, 
             color = week, 
             label = tree)) +
...
ggsave(here("figures", "qudo figures", "vardecomp_combined"), combined_plot2, device = "jpg", width = 7, height = 5, dpi = 300)
```
