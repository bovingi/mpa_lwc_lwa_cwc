...
#to run variance decompositions: 
...
wc_all_raw <- read_csv(here("processed-data", paste0("analysis",datver,".csv")), show_col_types = F) %>% 
  filter(!(week %in% 33)) #week 33 is screwy for LWA for some reason, remove. We do this in the main analysis too. Likely issue with scanning or scale not being tared. 
```
...
m2 <- lmer(water_potential ~ lwc_mean + (1|tree), data = qudo_wc_all)
m3 <- lmer(water_potential ~ week + lwc_mean + (1|tree), data = qudo_wc_all)
...
mvar <- lmer(water_potential ~ lwc_mean + (1|site/tree) + (1|week), data=qudo_wc_full)
...
mvar_lin <- lmer(water_potential ~ lwc_mean + (1|site/tree) + (1|week), data=qudo_wc_linear)
...
#ggsave(here("figures",  "qudo figures", "LWC",  "vdc2_plot"), vdc2_plot_lwc, device = "jpg", width = 5.5, height = 7, dpi = 300)
```
...
#ggsave(here("figures", "qudo figures", "LWC", "vardecomp_lma_plot"), vardecomp_lma_plot, device = "jpg", width = 12, height = 5, dpi = 300)
```
...
var(qudo_wc_linear$water_potential, na.rm=T) # 0.9556679
vest_lin <- data.frame(VarCorr(mvar_lin))
...
#ggsave(here("figures", "qudo figures", "LWA", "vdc2_lwa_new_plot"), vdc2_plot, device = "jpg", width = 5.5, height = 7, dpi = 300)
```
...
#ggsave(here("figures", "qudo figures", "vardecomp_combined"), combined_plot2, device = "jpg", width = 7, height = 5, dpi = 600)
```
