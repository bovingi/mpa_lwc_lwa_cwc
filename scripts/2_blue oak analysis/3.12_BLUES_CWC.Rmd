...
data_qudo_lma <- read_csv(here("processed-data", paste0("wp_wc_rwc_",datver,".csv"))) %>% 
 # filter(time %in% c("md")) %>% 
  ...
  filter(species %in% c("blue oak")) %>% 
  select(week, tree, species, lma_g_cm2, water_potential, lwc_mean, lwa_g_cm2) %>% 
  distinct()
```
...
# dcwc_df_no_outliers %>% 
#   filter((date_wp %in% c(sig_mods_before_mpa$date_wp) & time %in% c(sig_mods$time))) %>% 
#                    ggplot(aes(y = slope, 
#                               x = water_potential, 
#                               color = time)) +
#                    geom_smooth(method = "lm") +
...
# dcwc_df_no_outliers %>% 
#   filter((date_wp %in% c(sig_mods_all_mpa$date_wp) & time %in% c(sig_mods$time))) %>% 
#                    ggplot(aes(y = slope, 
#                               x = water_potential, 
#                               color = time)) +
#                    geom_smooth(method = "lm") +
...
```{r}
sig_mods_all <- bind_rows(sig_mods_all_mpa %>% mutate(var = "mpa",
...
write_csv(sig_mods_all, here("processed-data", "model results", "dcwc_mods.csv"))
```
