...
```{r, eval = F}
#JEans new wavelength data:
# Define the folder path containing the CSVs
folder_path <- here::here("processed-data", "SHIFT_data", "cwc_1100_1300")

# List all CSV files in the folder
csv_files <- list.files(folder_path, pattern = "\\.csv$", full.names = TRUE)

# Function to read each CSV and add a column with numbers extracted from the filename
read_and_label <- function(file) {
  df <- read_csv(file)
  # Extract numbers from the filename
  file_number <- str_extract(basename(file), "\\d+")
  # Add new column with extracted numbers
  df <- df %>% mutate(file_number = file_number)
  return(df)
}

# Read and combine all CSVs
data_jean_cwcdf0  <- map_df(csv_files, read_and_label) 

data_jean_cwcdf <- data_jean_cwcdf0 %>% 
  clean_names() %>% 
  mutate(date = file_number, 
         date_shift = ymd(date), 
         tree = tree_number) %>% 
  select(tree, date_shift, cwc)

data_og_cwc0 <- read_csv(here("processed-data", paste0("cwc_analysis",datver,".csv")), show_col_types = F) %>%  
  filter(species %in% c("blue oak")) %>% 
  select(tree, week, water_potential, time_season, time, site, date_wp, date_shift) %>% 
  mutate(water_potential = -1*water_potential) %>% 
  distinct() %>% 
  filter(!(tree %in% c(2346))) %>% 
  filter(!week %in% c(11, 9, 18)) 


data_og_cwc <- merge(data_og_cwc0, data_jean_cwcdf, by = c("tree", "date_shift"), all.x = TRUE)

unique(data_og_cwc2$date_shift)
```
...
ggsave(here("figures", "qudo figures", "CWC", "combined_plots_all_qudo_bs.jpg"), 
       combined_plots_all_legend, 
       device = "jpeg", 
