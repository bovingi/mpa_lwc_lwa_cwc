...
# Function to calculate the slope for a specific tree within each iteration
calculate_slope_for_tree <- function(tree_data) {
  model <- lm(week ~ cwc, data = tree_data)
  return(coef(model)[2])  # Extract the slope coefficient
}
...
model_results <- filtered_data %>%
  ...
  group_modify(~ tidy(lmer(water_potential ~ slope, data = .x))) %>%
  filter(term == "slope") %>%
...
data %>% 
  ...
  geom_smooth(method = "lm", se = F)
```
Should filter out the trees that were actually measured over the whole time period (e.g. up until September)
...
# Number of random trees to sample
n_sample_trees <- 40  # Adjust as needed based on the number of unique trees
...
```
####Psi ~ CWC as dCWC: 
Are there patterns in spatial dCWC that map onto MPa? 

```{r}
dcwc_space <- tbootstrapped_slopes_df_after %>% 
  select(tree, slope) %>% 
  distinct()

dcwc_df <- merge(dcwc_space, data_og_cwc, by = c("tree"))
...
m1 <- lm(slope ~ water_potential, data = dcwc_pd)
summary(m1)
anova(m1)
plot(m1)
```
...
m1 <- lm(slope ~ water_potential, data = dcwc_md)
summary(m1)
anova(m1)
...
m1 <- lm(slope ~ log(-1*water_potential), data = dcwc_md)
summary(m1)
anova(m1)
...
data_dcwc <- data_og_cwc %>% 
  mutate(week = as.character(week)) %>% 
  pivot_wider(names_from = week, 
              values_from = c(cwc, water_potential)
              ) %>% 
  select(-date_wp, -time_season) %>% 
  group_by(tree, time) %>% 
  fill(c(3:25), .direction = "downup") %>% 
  distinct() %>% 
  ungroup() %>% 
  group_by(tree) %>% 
  fill(c(4:11), .direction = "downup") %>% 
  filter(time == "md") %>%#just middays for now! 
  mutate(week10_12 = cwc_10-cwc_12,
         week12_14 = cwc_12-cwc_14,
         week14_15 = cwc_14-cwc_15,
         week15_17 = cwc_15-cwc_17,
         week17_18 = cwc_17-cwc_18,
         week18_19 = cwc_18-cwc_19,
         week19_21 = cwc_19-cwc_21,
         week21_33 = cwc_21-cwc_33,
         week33_37 = cwc_33-cwc_37,
         )
