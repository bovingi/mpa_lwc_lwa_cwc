...
# Define function to calculate the slope for a specific tree
...
# Define function for a single bootstrap iteration
bootstrap_iteration <- function(data, sample_size) {
  # Sample trees with replacement
  sampled_tree_ids <- sample(unique(data$tree), size = sample_size, replace = TRUE)
  ...
  # Return mean slope for this iteration
  mean(slopes, na.rm = TRUE)
}

# Bootstrap analysis for each time season
run_bootstrap <- function(data, time_season, weeks) {
  # Filter data for the specific weeks and time season
  filtered_data <- data %>%
  ...
  bootstrapped_results <- future_map_dbl(1:n_iterations, ~bootstrap_iteration(filtered_data, n_sample_trees), .progress = TRUE)
  
  # Calculate confidence intervals
  ci_values <- tibble(
    mean = mean(bootstrapped_results, na.rm = TRUE),
    lower_bound = quantile(bootstrapped_results, 0.025, na.rm = TRUE),
    upper_bound = quantile(bootstrapped_results, 0.975, na.rm = TRUE),
    pre_post = time_season
  )
  
  ci_values
}

# Combine results for before and after leafout
time_boot_df <- bind_rows(
  run_bootstrap(filtered_data, "before leafout", 10:17),
  run_bootstrap(filtered_data, "after leafout", 18:40)
) %>%
...
```{r}
mod <- lmer(water_potential ~ lwa_g_cm2_new*week + (1|site), data = data %>% 
              mutate(week = as.factor(week)))
...
ggsave(here("figures", "qudo figures", "LWA", "nice_space"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
ggsave(here("figures", "qudo figures", "LWA", "nice_space_centered"), space_nice_centered, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
ggsave(here("figures", "qudo figures", "LWA", "time_summary_slopes_boot_tree"), time_summary_bs_tree, device = "jpg", width = 2.5, height = 3, dpi = 300)
```
...
combined_slopes_wide <- cowplot::plot_grid( hyd_slopes, NULL, time_summary, NULL,space_slope_bs,  
                                           # NULL, pv_slope,
                                   nrow = 1, 
                                  rel_widths = c(1, -.02, 1, -.02, 1.25 #, -.02, .5
                                                 ), 
                                  labels = c("D", "", "E", "", "F"#,"", "G"
                                             ),
                                  label_x = .2,
...
ggsave(here("figures", "qudo figures", "LWA", "combined_plots_all_qudo_new"), combined_plots_all, device = "jpeg", width = 12, height = 8, dpi = 300)
```
