...
```{r}
data_qudo_raw <-  read_csv(here("processed-data", paste0("analysis_bothspp",datver,".csv")))  %>% 
...
write_csv( summary_df_hyd, here::here("processed-data", " summary_df_hyd_qudo_lwc.csv"))
```
#COMBINED DF: 
...
```
#-----------------
Since the above takes a long time to run, can just read in completed data from prev. runs (should be the same):


#START HERE to skip bootstrapping: 

```{r}
final_df_all <- read_csv(here::here("processed-data", "bootstrapped_df_qudo_lwc_mean.csv"))
```
...
hyd_mod_df <- broom.mixed::tidy(m_wk_rand)
```
...
Add LMA model to see if just using phenology or LMA is better than using water related term. 
```{r}
data_qudo_lma <- data_qudo_raw %>% 
 # filter(time %in% c("md")) %>% 
  ...
  filter(species %in% c("blue oak")) %>% 
  filter(time == "md") %>% 
  select(week, tree, species, lma_g_cm2, water_potential, time_season)

lma_mod <- lmer(water_potential ~ lma_g_cm2 + (lma_g_cm2|week) , data = data_qudo_lma, REML = T)
lma_mod

broom.mixed::glance(lma_mod)
r.squaredGLMM(lma_mod) 

lma_mod_time_int <- lmer(water_potential ~ lma_g_cm2*time_season + (1|week) + (lma_g_cm2|tree), data = data_qudo_lma)
lma_mod_time_int

broom.mixed::glance(lma_mod_time_int)
r.squaredGLMM(lma_mod_time_int) 

lma_mod_time <- lmer(water_potential ~ lma_g_cm2 + time_season + (1|week) + (lma_g_cm2|tree), data = data_qudo_lma)
lma_mod_time

broom.mixed::glance(lma_mod_time)
r.squaredGLMM(lma_mod_time) 

anova(lma_mod_time_int, lma_mod_time)
```
...
glance_df <- bind_rows(space_glance <- broom.mixed::glance(space_mod1) 
                       %>% mutate(mod = "space"),
                       lma_space_glance <- broom.mixed::glance(lma_mod) 
                       %>% mutate(mod = "lma_space"),
                    ...
                    as_data_frame(r.squaredGLMM(time_season_mod)) 
                       ...
                       %>% mutate(mod = "interaction"),
                    lma_time_glance <- broom.mixed::glance(lma_mod_time) 
                       %>% mutate(mod = "lma_time_interaction"),
                    as_data_frame(r.squaredGLMM(lma_mod_time)) 
                       %>% mutate(mod = "lma_time_interaction"),
                    as_data_frame(r.squaredGLMM(time_season_mod)) 
                       %>% mutate(mod = "lma_space")
                       ) %>% 
...
#*!START HERE FOR JUST FIG CHANGES!
```{r}
...
hyd_slopes <- final_df_all %>% 
  ...
  geom_rect(aes(ymin = 12.3, ymax = 18, xmin = -Inf, xmax = Inf), 
            fill = "#969696", alpha = .01, color = NA) +
...
highlighted_weeks <- c("03-08", "03-15", "03-25",
                       "04-04", "04-11", "05-09", 
                       "05-23", "07-19", "08-18", "09-15")  # Example weeks
...
time_summary <- final_df_all %>% 
  ...
  geom_rect(aes(ymin = 12.3, ymax = 18, xmin = -Inf, xmax = Inf), 
            fill = "#969696", alpha = 0.05, color = NA) +
...
library(grid)
#labels of slopes: 
y.grob1 <- textGrob("Slope (\U0394 \U03A8 / \U0394 LWmass)", 
...
ggsave(here("figures", "qudo figures", "LWC", "combined_plots_all_qudo_bs.jpg"), combined_plots_all, device = "jpg", width = 12, height = 7, dpi = 600)
```
...
```{r}
lma_mpa_df <- read_csv(here::here("data", "wp_wc_df.csv")) %>% 
...
annotate(geom = "text", 
         ...
         y = -4.75, 
         color =  "#165E6F",
         label = bquote(R^2 == .(marginal_r2_pd))) +
annotate(geom = "text", 
         x = .01, 
         y = -5, 
         color = "#CC5265",
         label = bquote(R^2 == .(marginal_r2_md))) +
  theme(#legend.position="top",
 ...
 labs(
  y = "Water Potential (MPa)",
  x = expression("Leaf Mass per Area (g/cm"^2*")")
) +
  scale_color_manual(values = c("#CC5265","#165E6F"), 
                     name = "Time", 
...
ggsave(here("figures", "qudo figures", "LWC", "lma_mpa_plot.jpg"), lma_mpa_plot, device = "jpg", width = 6, height = 4, dpi = 600)
...
lma_df <- bind_rows(lma_alas_df, lma_mpa_df %>% 
                           select(-week)) %>% 
  filter(lma_g_cm2_new < 0.025) %>% 
  distinct() 
#PD alone: 
...
#ggsave(here("figures", "qudo figures", "LWC", "lma_mpa_plot_alas.jpg"), lma_mpa_plot_alas, device = "jpg", width = 6, height = 4, dpi = 600)
```
