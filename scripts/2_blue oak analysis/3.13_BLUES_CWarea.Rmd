---
title: "Establishing relationships between cw_area and MPa, Part 2"
author: "Indra Boving"
date: "11/9022"
output:
...
Pull 41 trees, get mean slope for those 41, do 1000x. 
...
data <- filtered_data  %>% 
  filter(time_season == "before leafout") %>% 
  distinct() %>% 
  drop_na(water_potential, cw_area) %>% 
  group_by(tree) %>% 
  mutate(COUNT = n()) %>% 
  filter(!(COUNT == 1)) %>% 
  select(-COUNT)
...
calculate_slope_for_tree <- function(tree_data) {
  model <- lm(water_potential ~ cw_area, data = tree_data)
...
data <- common_trees_data %>% 
  filter(time_season == "after leafout") %>% 
  select(tree, week, water_potential, cw_area) %>% 
  distinct() %>% 
  drop_na(water_potential, cw_area) %>% 
  group_by(tree) %>% 
  mutate(COUNT = n()) %>% 
...
data <- df %>% 
  filter(time == "md") %>% 
  select(tree, week, water_potential, cw_area, site) %>% 
  distinct() %>% 
  drop_na(cw_area, water_potential)
...
#model without site: 
space_mod_4 <- lmer(water_potential ~ cw_area + (cw_area|week), data =  data , REML = T)
...
for (x in unique_weeks) {
  ...
  for (i in 1:n_iterations) {
    ...
    # Calculate the slope for the bootstrapped sample
    model <- lm(water_potential ~ cw_area, data = boot_sample)
...
final_df_all_cw_area %>% write_csv(here::here("processed-data", "bootstrapped_df_qudo_cw_area.csv"))
```
#------------

#START HERE to skip bootstrapping: 
```{r}
final_df_all_cw_area <- read_csv(here::here("processed-data", "bootstrapped_df_qudo_cw_area.csv"))
```
...
wk_all_wide <- wk_all %>% 
  ...
  pivot_wider(names_from=time, 
              values_from = c(cw_area, water_potential), 
              values_fn = function(x) mean(x, na.rm = TRUE)
              ) %>% 
    dplyr::group_by(tree, week) %>%
  fill(c(cw_area_md, cw_area_pd, water_potential_pd, water_potential_md), .direction = "downup") %>%
  dplyr::ungroup()%>% 
 # select(-date_wp) %>% 
  distinct()

wk_all_long_cw_area <- wk_all_wide %>% 
  drop_na(cw_area_pd, cw_area_md, water_potential_md, water_potential_pd) %>% 
...
wk_all_long <- merge(wk_all_long_mpa, wk_all_long_cw_area) %>% 
  distinct() %>% 
 # select(-cw_area_NA, -water_potential_NA) %>% 
...
#ggsave(here("figures", "qudo figures", "CWA", "nice_space_cw_area"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
space_mod <- lmer(water_potential ~ cw_area_centered + (cw_area_centered|week), data = space_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ cw_area_centered  + (cw_area_centered|week) + (1|site), data = space_df_nosite,  REML = T)
...
space_mod <- lmer(water_potential ~ cw_area + (cw_area|week), data = space_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ cw_area  + (cw_area|week) + (1|site), data = space_df_nosite,  REML = T)
#summary(space_mod_site)
...
```{r, echo=FALSE}
space_mod_4 <- lmer(water_potential ~ cw_area + (cw_area|week) + (1|site), data =  wc_wk_centered_df, REML = T)
...
```{r}
mod <- lmer(water_potential ~ cw_area*week + (1|site), data =wc_wk_centered_df %>% 
              mutate(week = as.factor(week)))
...
space_mod <- lmer(water_potential ~ cw_area + (1|site), data =  wk,  REML = T)
summary(space_mod)
#not sig
...
space_mod <- lm(water_potential ~ cw_area, data =  wk)
summary(space_mod)
#not sig
...
#ggsave(here("figures", "qudo figures", "cw_area", "nice_time"), time, device = "jpg", width = 4, height = 2, dpi = 300)
```
...
space_mod <- lmer(water_potential ~ cw_area + (1|week) + (cw_area|tree), data = space_df_nosite, REML = F)
#summary(space_mod)
AIC(space_mod)

space_mod_site <- lmer(water_potential ~ cw_area  + (1|week) + (cw_area|tree) + (1|site), data = space_df_nosite,  REML = F)
...
time_season_mod <- lmer(water_potential ~ cw_area + time_season + (1|week) + (cw_area|tree), data = wc_qudo_md_df)

time_season_mod_df <- broom::tidy(time_season_mod)

#with separate periods: 
time_season_mod_int <- lmer(water_potential ~ cw_area*time_season + (1|week) + (cw_area|tree), data = wc_qudo_md_df)

time_season_mod_int_df <- broom::tidy(time_season_mod)

time_mod <- lmer(water_potential ~ cw_area + (1|week) + (cw_area|tree), data = wc_qudo_md_df)
#time_mod <- lmer(water_potential ~ cw_area + (cw_area|tree), data = wc_qudo_md_df)
...
lin_time_mod <- lmer(water_potential ~ cw_area + (1|week) + (cw_area|tree), data = lin_wc_qudo_md_df)
...
bef_time_mod <- lmer(water_potential ~ cw_area + (1|week) + (cw_area|tree), data = bef_wc_qudo_md_df, REML = T)
...
#ggsave(here("figures", "qudo figures", "cw_area", "time_summary_slopes"), time_summary, device = "jpg", width = 2.5, height = 3, dpi = 300)
```
...
#ggsave(here("figures", "qudo figures", "cw_area", "time_summary_slopes_boot_tree"), time_summary_bs_tree, device = "jpg", width = 2.5, height = 3, dpi = 300)
```
...
write_csv(cw_area_mods, here::here("processed-data", "model results", "cw_area_mods.csv"))
```
...
legend_plot <- df  %>% 
  ungroup() %>% 
  add_row(date_wp = as.Date("2022-05-03"), time = "md", cw_area = .05, water_potential = -.3) %>% 
...
library(grid)
#labels of slopes: 
y.grob1 <- textGrob("Slope (\U0394 \U03A8 / \U0394 CWarea)",
...
x.grob2 <- textGrob("Canopy Water (g/cm2)", 
                   gp=gpar(fontface="bold", #col="blue",
                           fontsize=15))
...
#ggsave(here("figures", "qudo figures", "cw_area", "combined_plots_all_qudo_bs"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
