...
final_df_all_cw_area %>% write_csv(here::here("processed-data", "bootstrapped_df_qudo_cwc_cw_area.csv"))
```
#------------

#START HERE to skip bootstrapping: 
```{r}
final_df_all_cw_area <- read_csv(here::here("processed-data", "bootstrapped_df_qudo_cwc_cw_area.csv"))
```
...
ggsave(here("figures", "qudo figures", "cw_area", "cw_area", "nice_space_cwc_cw_area"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
time_season_mod <- lmer(cwc ~ cw_area + time_season + (1|week) + (cw_area|tree), data = wc_qudo_md_df)

time_season_mod_df <- broom::tidy(time_season_mod)

#with separate periods: 
time_season_mod_int <- lmer(cwc ~ cw_area*time_season + (1|week) + (cw_area|tree), data = wc_qudo_md_df)

time_season_mod_int_df <- broom::tidy(time_season_mod)

time_mod <- lmer(cwc ~ cw_area + (1|week) + (cw_area|tree), data = wc_qudo_md_df)
#time_mod <- lmer(cwc ~ cw_area + (cw_area|tree), data = wc_qudo_md_df)
...
ggsave(here("figures", "qudo figures", "cw_area", "combined_plots_all_qudo_cwc_bs"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
