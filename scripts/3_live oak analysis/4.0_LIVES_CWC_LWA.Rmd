...
#LWA data: 
data_og_lwa  <- read_csv(here::here("data", paste0("analysis_bothspp",datver,".csv")), show_col_types = FALSE) %>% 
...
```{r, eval = F}
space_slope_bs <-  final_df_all_cwc_lwa %>% 
   filter(!week %in% c(9)) %>% 
  filter(analysis == "space") %>% 
 # mutate(week = as.numeric(week)) %>% 
   ggplot(aes(color= as.factor(week))
    ) + 
  geom_point(aes(y = mean_slope ,x = as.factor(week)), size =2) +
    geom_point(aes(y = upr, x = as.factor(week)), size = 1) +
  geom_point(aes(y = lwr, x = as.factor(week)), size = 1) +
  #geom_line(aes(group = week, y = week, x = upr)) +
  geom_segment(aes(y= lwr, 
...
library(grid)
#labels of slopes: 
y.grob1 <- textGrob("Slope (\U0394 LWare/ \U0394 CWC)",
...
ggsave(here("figures", "quag figures", "CWC", "combined_plots_lwa_quag_bs"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
