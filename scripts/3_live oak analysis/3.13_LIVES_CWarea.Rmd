...
(Splitting into pre vs. post leafout doesn't work here, not enough reps in after before leafout)

Do we need an interaction between time_season and cw_area? 
```{r}
...
Ignoring phenology, what are the slopes per tree? 
```{r}
...
```{r}
cw_area_mods <- bind_rows(space_mod_df <- space_mod_df_4 %>% mutate(mod = "space"),
