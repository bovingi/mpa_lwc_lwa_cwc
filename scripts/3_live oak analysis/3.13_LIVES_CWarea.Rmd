...
##LMA, LWA, LWC data: 
data_quag_lma <- read_csv(here("processed-data", paste0("wp_wc_rwc_",datver,".csv"))) %>% 
 # filter(time %in% c("md")) %>% 
  group_by(tree, week) %>% 
...
df0 <- merge(data_quag_lma, data_og_cwc %>% select(-water_potential),
...
final_df_all_cw_area %>% write_csv(here::here("processed-data", "bootstrapped_df_quag_cw_area.csv"))
```
#------------

#START HERE to skip bootstrapping: 
```{r}
final_df_all_cw_area <- read_csv(here::here("processed-data", "bootstrapped_df_quag_cw_area.csv"))
```
...
ggsave(here("figures", "quag figures", "cw_area", "cw_area", "nice_space_cw_area"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
time_season_mod <- lmer(water_potential ~ cw_area + time_season + (1|week) + (cw_area|tree), data = wc_quag_md_df)

time_season_mod_df <- broom::tidy(time_season_mod)

#with separate periods: 
time_season_mod_int <- lmer(water_potential ~ cw_area*time_season + (1|week) + (cw_area|tree), data = wc_quag_md_df)

time_season_mod_int_df <- broom::tidy(time_season_mod)

time_mod <- lmer(water_potential ~ cw_area + (1|week) + (cw_area|tree), data = wc_quag_md_df)
#time_mod <- lmer(water_potential ~ cw_area + (cw_area|tree), data = wc_quag_md_df)
...
lin_time_mod <- lmer(water_potential ~ cw_area + (1|week) + (cw_area|tree), data = lin_wc_quag_md_df)
...
bef_time_mod <- lmer(water_potential ~ cw_area + (1|week) + (cw_area|tree), data = bef_wc_quag_md_df, REML = T)
...
ggsave(here("figures", "quag figures", "cw_area", "time_summary_slopes"), time_summary, device = "jpg", width = 2.5, height = 3, dpi = 300)
```
...
write_csv(cw_area_mods, here::here("processed-data", "model results", "cw_quag_area_mods.csv"))
```
...
ggsave(here("figures", "quag figures", "cw_area", "combined_plots_all_quag_bs"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
