...
dcwc_df_no_outliers %>% write_csv(here("processed-data", "dcwc_quke_afterleafout_df_no_outliers.csv"))
```
...
dcwc_df_no_outliers %>% write_csv(here("processed-data", "dcwc_quke_beforeleafout_df_no_outliers.csv"))
```
...
dcwc_df_no_outliers %>% write_csv(here("processed-data", "dcwc_quke_justmay_df_no_outliers.csv"))
```
...
dcwc_df_no_outliers %>% write_csv(here("processed-data", "dcwc_quke_wholedaterange_df_no_outliers.csv"))
```
#TABLE: 
...
dcwc_wholedaterange <- read_csv(here("processed-data", "dcwc_quke_wholedaterange_df_no_outliers.csv")) 

dcwc_beforeleafout <- read_csv(here("processed-data", "dcwc_quke_beforeleafout_df_no_outliers.csv")) 

dcwc_afterleafout <- read_csv(here("processed-data", "dcwc_quke_afterleafout_df_no_outliers.csv")) 

dcwc_justmay <- read_csv(here("processed-data", "dcwc_quke_justmay_df_no_outliers.csv")) 

#lma, mpa, and lwmass and lwarea data: 
data_quke_lma <- read_csv(here("processed-data", paste0("wp_wc_rwc_",datver,".csv"))) %>% 
 # filter(time %in% c("md")) %>% 
  group_by(tree, week) %>% 
...
dcwc_alldata <- merge(dcwc_wholedaterange, data_quke_lma, by = c("tree", "week"), all = T) %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
  ungroup()

dcwc_beforeleafout<- merge(dcwc_beforeleafout , data_quke_lma, by = c("tree", "week"), all = T) %>% 
  distinct() %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
...
dcwc_afterleafout<- merge(dcwc_afterleafout , data_quke_lma, by = c("tree", "week"), all = T) %>% 
  distinct() %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
...
dcwc_justmay <-  merge(dcwc_justmay, data_quke_lma, by = c("tree", "week"), all = T) %>% 
  distinct() %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
...
dcwc_allanalyses <- bind_rows(dcwc_afterleafout, dcwc_beforeleafout, dcwc_wholedaterange, dcwc_justmay)
```
...
write_csv(sig_mods_all %>%
   mutate(across(where(is.numeric), ~ round(., 3))), here(here("processed-data", "model results", "table_quke_sx.csv")))
...
mpa_plot <- fig_data %>% 
  ...
  facet_wrap(~date_wp, scales = "free", nrow = 5) +
...
# Save plot
ggsave(here::here("figures", "quke figures", "dcwc_mpa_alldates_SI.jpg"), plot = mpa_plot, width = 10, height = 10)
```
...
# Save plot
ggsave(here::here("figures", "quke figures", "dcwc_SI.jpg"), plot = final_plot, width = 8, height = 8)
```
