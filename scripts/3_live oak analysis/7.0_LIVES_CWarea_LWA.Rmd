...
#Actually use this, it has fewer steps ot get in working order
...
final_df_all_cw_area %>% write_csv(here::here("processed-data", "bootstrapped_df_quag_lwa_cw_area.csv"))
```
#------------

#START HERE to skip bootstrapping: 
```{r}
final_df_all_cw_area <- read_csv(here::here("processed-data", "bootstrapped_df_quag_lwa_cw_area.csv"))
```
...
ggsave(here("figures", "quag figures", "cw_area", "cw_area", "nice_space_lwa_cw_area"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
```{r, echo=FALSE}
space_mod_4 <- lmer(lwa_g_cm2 ~ cw_area + (cw_area|week), data =  wc_wk_centered_df, REML = T)
...
time_season_mod <- lmer(lwa_g_cm2 ~ cw_area + time_season + (1|week) + (cw_area|tree), data = wc_quag_md_df)

time_season_mod_df <- broom::tidy(time_season_mod)

#with separate periods: 
time_season_mod_int <- lmer(lwa_g_cm2 ~ cw_area*time_season + (1|week) + (cw_area|tree), data = wc_quag_md_df)

time_season_mod_int_df <- broom::tidy(time_season_mod)

time_mod <- lmer(lwa_g_cm2 ~ cw_area + (1|week) + (cw_area|tree), data = wc_quag_md_df)
#time_mod <- lmer(lwa_g_cm2 ~ cw_area + (cw_area|tree), data = wc_quag_md_df)
...
ggsave(here("figures", "quag figures", "cw_area", "combined_plots_all_quag_lwa_bs"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
