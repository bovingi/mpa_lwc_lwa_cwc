...
final_df_all_cw_area %>% write_csv(here::here("processed-data", "bootstrapped_df_quag_lwc_cw_area.csv"))
```
#------------

#START HERE to skip bootstrapping: 
```{r}
final_df_all_cw_area <- read_csv(here::here("processed-data", "bootstrapped_df_quag_lwc_cw_area.csv"))
```
...
ggsave(here("figures", "quag figures", "cw_area", "cw_area", "nice_space_lwc_cw_area"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
time_season_mod <- lmer(lwc_mean ~ cw_area + time_season + (1|week) + (cw_area|tree), data = wc_quag_md_df)

time_season_mod_df <- broom::tidy(time_season_mod)

#with separate periods, interaction: 
time_season_mod_int <- lmer(lwc_mean ~ cw_area*time_season + (1|week) + (cw_area|tree), data = wc_quag_md_df)
...
```{r}
#Not using phenology at all
time_mod <- lmer(lwc_mean ~ cw_area + (1|week) + (cw_area|tree), data = wc_quag_md_df)
#time_mod <- lmer(lwc_mean ~ cw_area + (cw_area|tree), data = wc_quag_md_df)
...
write_csv(cw_area_mods, here::here("processed-data", "model results", "quag results", "cw_area_lwc_mods.csv"))
```
...
ggsave(here("figures", "quag figures", "cw_area", "combined_plots_all_quag_lwc_bs"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
