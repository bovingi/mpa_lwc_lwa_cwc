...
dcwc_df_no_outliers %>% write_csv(here("processed-data", "dcwc_quag_afterleafout_df_no_outliers.csv"))
```
...
dcwc_df_no_outliers %>% write_csv(here("processed-data", "dcwc_quag_beforeleafout_df_no_outliers.csv"))
```
...
dcwc_df_no_outliers %>% write_csv(here("processed-data", "dcwc_quag_justmay_df_no_outliers.csv"))
```
...
dcwc_df_no_outliers %>% write_csv(here("processed-data", "dcwc_quag_wholedaterange_df_no_outliers.csv"))
```
#COMBINE:
...
dcwc_wholedaterange1 <- read_csv(here("processed-data", "dcwc_quag_wholedaterange_df_no_outliers.csv")) %>% 
  mutate(analysis = "whole")


dcwc_beforeleafout1 <- read_csv(here("processed-data", "dcwc_quag_beforeleafout_df_no_outliers.csv"))%>% 
  mutate(analysis = "before")
 

dcwc_afterleafout1 <- read_csv(here("processed-data", "dcwc_quag_afterleafout_df_no_outliers.csv")) %>% 
  mutate(analysis = "after")


dcwc_justmay1 <- read_csv(here("processed-data", "dcwc_quag_justmay_df_no_outliers.csv")) %>% 
...
#So that dates line up across all analyses:
  ...
  mutate(date_wp = ymd(date_new)) 
#Only want dates that we have good quag data for:
...
```{r}
dcwc_alldata <- merge(dcwc_wholedaterange1, data_quag_lma, by = c("tree", "week", "date_wp", "date_new"), all = T) %>% 
  distinct() %>%
  group_by(tree) %>% 
...
dcwc_beforeleafout<- merge(dcwc_beforeleafout1 , data_quag_lma, by = c("tree", "week","date_wp", "date_new"), all = T) %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
  ungroup() %>% 
...
dcwc_afterleafout<- merge(dcwc_afterleafout1, data_quag_lma, by = c("tree", "week", "date_wp", "date_new"), all = T) %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
  ungroup() %>% 
...
dcwc_justmay <-  merge(dcwc_justmay1, data_quag_lma, by = c("tree", "week", "date_wp", "date_new"), all = T) %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
  ungroup() %>% 
...
write_csv(
  ...
  here("processed-data", "model results", "table_quag_sx.csv")
)
...
# Save plot
#ggsave(here::here("figures", "quag figures", "dCWC",  "dcwc_mpa_alldates_SI.jpg"), plot = mpa_plot, width = 10, height = 5)
```
...
# Save plot
#ggsave(here::here("figures", "quag figures", "dCWC",  "dcwc_SI.jpg"), plot = final_plot, width = 8, height = 8)
```
...
write_csv(
  ...
  here::here("processed-data", "model results", "table_quag_sx.csv")
)
...
ggsave(here::here("figures", "quag figures", "dCWC",  "means_plot_SI.jpg"), plot = means_plot, width = 6, height = 8)
```
...
# Save plot
#ggsave(here::here("figures", "quag figures", "dCWC",  "dcwc_SI.jpg"), plot = final_plot, width = 6.5, height = 8)
```
...
# Save plot
#ggsave(here::here("figures", "quag figures", "dCWC",  "dcwc_combinedspp_SI.jpg"), plot = final_plot, width = 6.5, height = 8)
```
