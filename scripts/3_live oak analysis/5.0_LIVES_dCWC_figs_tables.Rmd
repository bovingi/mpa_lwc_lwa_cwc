...
```{r}
dcwc_alldata <- merge(dcwc_wholedaterange1, data_quag_lma, by = c("tree", "week","time", "water_potential", "date_wp"), all = T) %>% 
  distinct() %>%
  group_by(tree) %>% 
...
dcwc_beforeleafout<- merge(dcwc_beforeleafout1 , data_quag_lma, by = c("tree", "week","time", "water_potential", "date_wp"), all = T) %>% 
  distinct() %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
...
dcwc_afterleafout<- merge(dcwc_afterleafout1, data_quag_lma, by = c("tree", "week","time", "water_potential", "date_wp"), all = T) %>% 
  distinct() %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
...
dcwc_justmay <-  merge(dcwc_justmay1, data_quag_lma, by = c("tree", "week", "time", "water_potential", "date_wp"), all = T) %>% 
  distinct() %>% 
  distinct() %>%
  group_by(tree) %>% 
  fill(c(slope), .direction = "downup") %>% 
