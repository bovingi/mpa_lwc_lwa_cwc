...
```{r, eval}
data_og_quag_cwc <- read_csv(here("processed-data", paste0("cwc_analysis",datver,".csv")), show_col_types = F) %>%  
  filter(species %in% c("live oak")) %>% 
  select(tree, week, water_potential, cwc, time_season, time, site, date_wp) %>% 
  distinct() %>% 
  mutate(water_potential = -1*water_potential) %>% 
...
```
Odd that some trees never have CWC: 
tree 2376
tree 2380 for week 13
...
model_summary_lives <- model_summary %>% arrange(delta_AIC) %>%
  mutate(
    delta_AIC = round(delta_AIC, 3)
  )
print(model_summary_lives)

write_csv(model_summary_lives, here("processed-data", "model results", "quke_mems.csv"))
```
...
n_sample_trees <- 14  # Adjust as needed based on the number of unique trees
...
# Number of random trees to sample
n_sample_trees <- 13  # Adjust as needed
...
```{r}
data <- data_og_quag_cwc %>% 
  filter(time == "md") %>% 
  select(tree, week, water_potential, cwc, site) %>% 
  distinct() %>% 
  drop_na(cwc, water_potential, site)

space_mod_4 <- lmer(water_potential ~ cwc + (cwc|week), data =  data , REML = T)

space_mod_3 <- lmer(water_potential ~ cwc + (cwc|week)  + (1|site), data =  data , REML = T)
...
Don't include site: 
```{r}
space_mod <- lmer(water_potential ~ cwc + (cwc|week), data =  data , REML = T)
...
space_mod_4 <- lmer(water_potential ~ cwc + (cwc|week) + (1|tree), data = wc_wk_centered_df, REML = T)
...
time_season_mod <- lmer(water_potential ~ cwc + time_season + (1|week) + (cwc|tree)+ (1|site), data = wc_quag_md_df)

time_season_mod_df <- broom::tidy(time_season_mod)

#with separate periods: 
time_season_mod_int <- lmer(water_potential ~ cwc*time_season + (1|week) + (cwc|tree)+ (1|site), data = wc_quag_md_df)

time_season_mod_int_df <- broom::tidy(time_season_mod)

anova(time_season_mod, time_season_mod_int)
```


```{r}
time_mod <- lmer(water_potential ~ cwc + (1|week) + (cwc|tree)+ (1|site), data = wc_quag_md_df)
#time_mod <- lmer(water_potential ~ cwc + (cwc|tree), data = wc_quag_md_df)
 
MuMIn::r.squaredGLMM(time_mod)

anova(time_season_mod, time_mod)
```
...
lin_time_mod <- lmer(water_potential ~ cwc + (1|week) + (cwc|tree) + (1|site), data = lin_wc_quag_md_df)
...
bef_time_mod <- lmer(water_potential ~ cwc + (1|week) + (cwc|tree) + (1|site), data = bef_wc_quag_md_df, REML = T)
...
write_csv(cwc_mods, here::here("processed-data", "model results", "cwc_quag_mods.csv"))
```
...
wc_wk_centered_df <- data_og_quag_cwc %>% 
  filter(time == "md") 
...
highlighted_weeks <- c(" ")  # Example weeks
  ...
  theme(legend.position="none",
    ...
    # axis.text.x  = element_blank(),
     ...
     legend.margin=margin(0,0,0,0),
      legend.box.margin=margin(-5,-8,-8,-8),
    plot.margin = unit(c(5.5, 5.5, 0, 5.5), units = "pt"),
     axis.text.x = element_text(size = 12, angle = 90, vjust = 0.5, hjust=.25)
    ) +
  #scale_x_continuous(breaks = c(10, 12, 14,15, 17, 19, 21, 29, 33, 37)
...
legend_plot <- data_og_cwc  %>% 
  ...
  add_row(date_wp = as.Date("2022-05-03"), time = "md", cwc = .05, water_potential = -.3, site = "Chamise") %>% 
  add_row(date_wp = as.Date("2022-05-03"), time = "pd", cwc = .05, water_potential = -.3, site = "Chamise") %>% 
  add_row(date_wp = as.Date("2022-05-03"), time = "md", cwc = .0456, water_potential = -.6, site = "Chamise")  %>% 
  add_row(date_wp = as.Date("2022-05-03"), time = "pd", cwc = .08, water_potential = -.6, site = "Chamise") %>%  
  add_row(date_wp = as.Date("2022-02-28"), time = "md", cwc = .05, water_potential = -.3, site = "Chamise") %>% 
  add_row(date_wp = as.Date("2022-02-28"), time = "pd", cwc = .05, water_potential = -.3, site = "Chamise") %>% 
  filter(date_wp %in% c(
         #"2022-02-28",
         "2022-03-08",
         "2022-03-15",
  ...
  labs(color = "Date", 
       shape = "Time") +
  scale_color_manual(values = c("02-28" ="#122241",
...
ggsave(here("figures", "quag figures", "CWC", "combined_plots_all_quag_new"), combined_plots_all_legend, device = "jpg", width = 10, height = 7, dpi = 300)
```
