...
```{r}
data_quag_raw <- read_csv(here::here("data", paste0("analysis_bothspp",datver,".csv")), show_col_types = FALSE) %>% 
...
data <- data_og_quag_lwc_mean %>% 
  ...
  filter(!(week %in% c(19, 15)))

space_mod_4 <- lmer(water_potential ~ lwc_mean + (lwc_mean|week) + (1|site), data =  data , REML = T)

space_mod_3 <- lmer(water_potential ~ lwc_mean + (lwc_mean|week), data =  data , REML = T)
...
final_df_space <- final_df_space %>% #this is bootstrapped; maybe we dont want to do that?
...
```{r}
final_df_all <- read_csv(here::here("processed-data", "bootstrapped_df_quag_lwc_mean.csv"))
```
...
```{r, eval = F}
#week 10: 
wk <- data_og_quag_lwc_mean %>% 
  drop_na(site) %>% 
  mutate(week = as.factor(week)) %>% 
  filter(week == 10)


space_mod <- lmer(water_potential ~ lwc_mean, data =  wk,  REML = T)
summary(space_mod)
...
space_mod <- lm(water_potential ~ lwc_mean, data =  wk)
summary(space_mod)
#p < 0.001
...
lma_mod <- lmer(water_potential ~ lma_g_cm2 + (lma_g_cm2|week) , data = data_quag_lma, REML = T)
lma_mod

broom.mixed::glance(lma_mod)
r.squaredGLMM(lma_mod) 

lma_mod_time <- lmer(water_potential ~ lma_g_cm2*time_season + (1|week) + (lma_g_cm2|tree), data = data_quag_raw)
lma_mod_time
...
mean_space <- final_df_all %>% 
  filter(analysis %in% c("space")) %>% 
  select(slope) %>% 
  mutate(mean_slope = mean(slope),
         sd = sd(slope))
mean_space

mean_time <- final_df_all %>% 
  filter(analysis %in% c("time, ind. tree")) %>% 
  select(slope, time_season) %>% 
...
highlighted_weeks <- c("04-25", "09-15")  # Example weeks
...
combined_slopes_wide <- cowplot::plot_grid(hyd_slopes , NULL, time_summary, NULL,
                                           space_slope_bs, #comment out the #_bs if want to use se  
                                           # NULL, pv_slope,
                                   nrow = 1, 
...
ggsave(here("figures", "quag figures", "LWC", "combined_plots_all_quag_new_bs"), combined_plots_all, device = "jpg", width = 12, height = 7, dpi = 300)
```
#------------------------------------
####SWC and LMA: 
...
```{r}
quag_lma_mpa_df <- read_csv(here::here("data", "wp_wc_df.csv")) %>% 
  filter(species == "live oak") %>% 
