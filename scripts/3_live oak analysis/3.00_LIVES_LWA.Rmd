...
```{r}
final_df_all <- read_csv(here::here("processed-data", "bootstrapped_df_quag_lwa_g_cm2_new.csv"))
```
...
wc_wk_centered_df <- data_og_quag_lwa %>% 
  filter(!week %in% c(9)) %>% 
  #filter(lwa_g_cm2_new > .44) %>% 
  mutate(water_potential = water_potential) %>% 
  filter(time == "md") %>% 
...
ggsave(file = here::here("figures", "quag figures", "LWA", "combined_plots_all_quag_new_bs.jpg"), plot = combined_plots_all, device = "jpeg", width = 12, height = 7, dpi = 300)
```
