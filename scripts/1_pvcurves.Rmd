...
```{r}
pv_new_rr_raw <- read_csv(here("data", "pv_curves_long_rayna.csv"), show_col_types = FALSE) %>% 
  clean_names()%>% 
...
```{r}
pv_new_kk_raw <- read_csv(here("data", "pv_shift_kk.csv"), show_col_types = FALSE) %>%
  clean_names() %>%
...
Older: 
```{r, echo=F}
pv_all_raw<- read_csv(here("data", "pv_curves_long.csv"), show_col_types = FALSE) %>% 
  clean_names() %>% 
