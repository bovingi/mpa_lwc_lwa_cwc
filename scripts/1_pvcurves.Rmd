...
```{r}
pv_new_rr_raw <- read_csv(here("final_data", "pv_curves_long_rayna.csv"), show_col_types = FALSE) %>% 
  clean_names()%>% 
...
df <- pv_new_rr_raw %>% 
  filter(rehydrated == "yes" & pd_or_md == "pd") %>% 
  dplyr::mutate(mean_tlp = mean(tlp, na.rm = T)) %>% 
  dplyr::mutate(sd_tlp = sd(tlp, na.rm = T)) %>% 
  select(mean_tlp, sd_tlp) %>% 
  distinct()
df
```
...
```{r}
pv_new_kk_raw <- read_csv(here("final_data", "pv_shift_kk.csv"), show_col_types = FALSE) %>%
  clean_names() %>%
...
Older: 
```{r, echo=F}
pv_all_raw<- read_csv(here("final_data", "pv_curves_long.csv"), show_col_types = FALSE) %>% 
  clean_names() %>% 
...
```{r}
data <- pv_new_df %>% 
  filter(date %in% c("2022-03-30", "2022-04-01")) %>% 
  distinct() %>% 
...
```{r}
data <- pv_new_df %>% 
  filter(date %in% c("2022-04-07", "2022-04-05")) %>% 
  distinct() %>% 
  drop_na(water_potential, lwc) %>% 
  mutate(tree = unique_id)
...
# Function to calculate the slope for a specific tree within each iteration
calculate_slope_for_tree <- function(tree_data) {
  model <- lm(water_potential ~ rwc, data = tree_data)
  return(coef(model)[2])  # Extract the slope coefficient
}
